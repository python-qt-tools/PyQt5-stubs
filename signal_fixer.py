...
def is_signal(module: object, cls_name: str, func_name: str) -> bool:
...
class TypingTransformer(cst.CSTTransformer):
    ...
    def __init__(self, mod_name: str):
        ...
        self._module = importlib.import_module(f"PyQt5.{mod_name}")
    ...
    def leave_FunctionDef(
        ...
        if is_signal(self._module, self._last_class[-1].name.value, f_name):
            ...
            if full_name in self._fixed_signals:
                # Handle the use-case of overloaded signals, i.e.:
                # QComboBox.highlighted
                return cst.RemovalSentinel.REMOVE
            self._fixed_signals.append(full_name)
            return cst.parse_statement(f"{f_name}: typing.ClassVar[QtCore.pyqtSignal]")
        return original_node
...
if __name__ == '__main__':
    for file in os.listdir("PyQt5-stubs"):
        if file.startswith("QtWebKit") or file in ["QtX11Extras.pyi", "sip.pyi", "__init__.pyi"]:
            continue
